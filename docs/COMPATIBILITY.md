...
Reviewed 11 September 2026. Baseline: release `0.2.0a1`. Improvements in this checkout: **unreleased `0.3.0a1`**. This review combines primary-source research, code inspection and isolated tests. It is not a certification by the MCP project, a penetration test or a promise that every agent application has been exercised.
...
| The historical expanded catalog has 136 tools | Exceeds some host/request limits before another MCP is added | Added `--tool-profile compact`: 42 workflow tools, with every discovered operation still reachable through `viewer_call`; default `all` preserves individual operations after refresh |
...
| Compact profile | Mock viewer refresh leaves 42 listed tools; generic operation call succeeds; it is a presentation option, not a permissions sandbox |
...
| Codex | Predecessor integration has inherited live usage. The new package/configuration requires a coordinated host/viewer check |
...
| Windows/Python | Prior release CI passed 3.11/3.12/3.13; this branch's CI must pass independently. Platform-independent wheel filename does not certify non-Windows viewer control |
...
1. **Coordinate a real upgrade test.** Use a free viewer session, isolated state and synthetic assets to verify the packaged LEAP launcher, MCP host, lease lifecycle, importer readback and image capture. Do not migrate an active consumer as part of a code/package test.
...
3. **Improve viewer evidence reliability.** Track precise UI search (basename/depth/offset), importer registry-button handling, focus-aware keys and blank-image detection. A consumer reported handled clicks without tab changes, full-path search hiding relevant matches behind a 200-result cap, and one entirely black but valid PNG. These are open defects/limitations; no live session was interrupted to investigate this audit.
...
5. **Harden distribution and permissions before a remote service.** The local bridge uses a random bearer token, loopback-only address validation, proxy bypass, Origin rejection and bounded bodies. It trusts the local user's filesystem and allowed callers. It is not a multi-tenant service, a general filesystem sandbox or a server-side policy engine for paid/chat/inventory operations. Adding HTTP/tunneling requires a deliberate authentication, session-isolation and authorization design.
...
- **Supply-chain improvements:** automate advisory checks, review dependency updates and consider signed/attested build artifacts. Checksums detect differences from a release; they are not publisher identity verification.
